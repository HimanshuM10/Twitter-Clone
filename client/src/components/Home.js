...
function Home() {
  ...
  useEffect(() => {
    ...
    const fetchPosts = async () => {
      try {
        const response = await axios.get('http://localhost:4000/api/posts/getPosts');
  ...
  const handleRepost = async (e, el) => {
    ...
    try {
      ...
      await axios.post('http://localhost:4000/api/posts/reTweet', {
  ...
  const toggleLikeDislike = async (e, el) => {
    ...
    setLoading(true);
    try {
      const result = await axios.put(`http://localhost:4000/api/posts/likePost/${el._id}/${loggedInUser?._id}`);
                  ...
                  <div
                    ...
                    <div
                      ...
                      <div
                        ...
                        {el?.postedBy?._id === loggedInUser?._id && ( // Display delete button if post created by logged-in user
                          <span style={{ marginLeft: '10px', cursor: 'pointer' }} onClick={() => handleDelete(el._id)}>
